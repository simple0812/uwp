...
namespace ListViewDemo.Libs
{
    public sealed class FileHelper
    {
        private static FileHelper _current;
        public static FileHelper Current => _current ?? (_current = new FileHelper());

        private readonly Windows.Storage.StorageFolder _local_folder;
        ...
        public async Task DeleteCacheFile()
        {
            try
            {
                ...
                if (folder != null)
                {
                    ...
                    List<IAsyncAction> list = files.Select(f => f.DeleteAsync(StorageDeleteOption.PermanentDelete)).ToList();
        ...
        public async Task<bool> CacheExist(string filename)
        {
            try
            {
                var p = await _local_folder.TryGetItemAsync("images_cache");
                var f = p as StorageFolder;
                if (f != null)
                {
                    var f2 = await f.TryGetItemAsync(filename);
                    return f2 != null;
                }
                return false;
            }
